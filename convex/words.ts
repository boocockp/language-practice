...
export async function lookupWordByText(
    ...
    const word = await ctx.db
        .query("words")
        .withIndex("by_userId_language", (q) => q.eq("userId", userId).eq("language", language))
...
export const getFirstByText = internalQuery({
    ...
    returns: v.union(v.null(), v.object({ text: v.string(), meaning: v.string() })),
    handler: async (ctx, args) => {
        const word = await ctx.db
            .query("words")
            .withIndex("by_userId_language", (q) => q.eq("userId", args.userId).eq("language", args.language))
 ...
 */
function matchesTags(wordTags: string | undefined, tagsOption: string): boolean {
...
export const getMatchingWordsByCriteria = internalQuery({
    ...
    handler: async (ctx, args) => {
        ...
        const textList = args.text && args.text.trim() !== "" ? parseCommaList(args.text) : null;
        const typeList = args.type && args.type.trim() !== "" ? parseCommaList(args.type) : null;
        const tagsOption = args.tags && args.tags.trim() !== "" ? args.tags : null;
        ...
        const matches = candidates.filter((word) => {
            ...
            if (tagsOption !== null && !matchesTags(word.tags, tagsOption)) return false;
            return true;
        });
...
export const getRandomByCriteria = internalAction({
    ...
    handler: async (
        ...
        const matches = await ctx.runQuery(internal.words.getMatchingWordsByCriteria, args);
