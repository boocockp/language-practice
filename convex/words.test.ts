...
async function insertWords(t: ReturnType<typeof convexTest>, userId: Id<"users">, language: string, words: string[]) {
...
describe("words.listByUserAndLanguage", () => {
    ...
    it("returns only the current user's words (user isolation)", async () => {
        ...
        const aWords = await userA.userSession.query(api.words.listByUserAndLanguage, { language: "en" });
        const bWords = await userB.userSession.query(api.words.listByUserAndLanguage, { language: "en" });
