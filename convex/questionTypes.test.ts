...
describe("questionTypes.listByUserAndLanguage", () => {
    ...
    it("returns empty array when authenticated but no question types exist", async () => {
        ...
        const result = await userSession.query(api.questionTypes.listByUserAndLanguage, { language: "en" });
        expect(result).toEqual([]);
    });
    ...
    it("filters by language", async () => {
        ...
        const enResult = await userSession.query(api.questionTypes.listByUserAndLanguage, { language: "en" });
        const frResult = await userSession.query(api.questionTypes.listByUserAndLanguage, { language: "fr" });
    ...
    it("returns only the current user's question types (user isolation)", async () => {
        ...
        const aList = await userA.userSession.query(api.questionTypes.listByUserAndLanguage, { language: "en" });
        const bList = await userB.userSession.query(api.questionTypes.listByUserAndLanguage, { language: "en" });
...
describe("questionTypes.create", () => {
    ...
    it("creates question type and returns id when authenticated", async () => {
        ...
        const doc = await t.run(async (ctx) => ctx.db.get("questionTypes", id));
...
describe("questionTypes.update", () => {
    ...
    it("does not update when unauthenticated", async () => {
        ...
        const doc = await t.run(async (ctx) => ctx.db.get("questionTypes", qtId));
