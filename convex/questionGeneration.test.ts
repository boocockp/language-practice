...
function defaultParams(overrides: Partial<GenerateQuestionParams> = {}): GenerateQuestionParams {
    return {
        dataTemplate: "",
        questionTemplate: "",
        answerTemplate: "",
...
describe("questionGeneration.generateQuestion", () => {
    ...
    it("word helper: stores word data for question/answer templates", async () => {
        const lookupWord: LookupWordFn = async (opts) =>
            opts.text === "chat" ? { _id: "w1", text: "chat", type: "nm", meaning: "cat" } : null;
    ...
    it("word helper with type only passes type to lookup", async () => {
        const lookupWord: LookupWordFn = async (opts) =>
            opts.type === "nm,nf" ? { _id: "w1", text: "chat", type: "nm", meaning: "cat" } : null;
    ...
    it("word helper with tags only passes tags to lookup", async () => {
        const lookupWord: LookupWordFn = async (opts) =>
            opts.tags === "abc&ghi,jkl" ? { _id: "w1", text: "xyz", type: "nm", meaning: "something" } : null;
    ...
    it("word helper with multiple options passes all to lookup", async () => {
        const lookupWord: LookupWordFn = async (opts) =>
            opts.text === "chat" && opts.type === "nm" ? { _id: "w1", text: "chat", type: "nm", meaning: "cat" } : null;
    ...
    it("async helper as subexpression: storeData awaits word result", async () => {
        const lookupWord: LookupWordFn = async (opts) =>
            opts.text === "dog" ? { _id: "w1", text: "dog", type: "nm", meaning: "chien" } : null;
