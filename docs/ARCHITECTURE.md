...
Detail forms (word edit, question type edit) use the shared **`DetailsFormShell`** component (`src/components/DetailsFormShell.tsx`). The shell provides the panel header (title + close), the discard-changes dialog, confirm-leave behavior (so the list page can prompt before leaving when dirty), and the Save/Cancel buttons. Each form (e.g. `WordDetailsForm`, `QuestionTypeDetailsForm`) keeps its own field state and validation and passes props into the shell; only the form fields are rendered as children. New list+detail data forms should use `DetailsFormShell` for consistent header, discard dialog, and actions.
