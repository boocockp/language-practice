...
describe("QuestionTypesTable", () => {
    ...
    it("renders column headers Name and Question template", () => {
        customRender(<QuestionTypesTable questionTypes={[]} selectedQuestionTypeId={null} onRowClick={() => {}} />);

        expect(screen.getByRole("columnheader", { name: "Name" })).toBeInTheDocument();
        expect(screen.getByRole("columnheader", { name: "Question template" })).toBeInTheDocument();
    });
    ...
    it("renders each question type with name and questionTemplate in cells", () => {
        ...
        customRender(
            <QuestionTypesTable questionTypes={questionTypes} selectedQuestionTypeId={null} onRowClick={() => {}} />,
        );
    ...
    it("calls onRowClick with question type _id when a row is clicked", async () => {
        ...
        customRender(
            <QuestionTypesTable questionTypes={questionTypes} selectedQuestionTypeId={null} onRowClick={onRowClick} />,
        );
        const rows = screen.getAllByRole("row").slice(1);
