...
function renderForm(overrides: Partial<React.ComponentProps<typeof QuestionTypeDetailsForm>> = {}) {
...
describe("QuestionTypeDetailsForm", () => {
    ...
    describe("new question type mode (questionType is null)", () => {
        ...
        it("calls onSave without questionTypeId when Save is clicked with valid fields", async () => {
            ...
            await user.type(screen.getByPlaceholderText("Descriptive name for this question type"), "New type");
            await user.type(screen.getByLabelText("Data template"), "data");
            await user.type(screen.getByLabelText("Question template"), "question");
            await user.type(screen.getByLabelText("Answer template"), "answer");
            await user.click(screen.getByRole("button", { name: "Save" }));
            expect(onSave).toHaveBeenCalledTimes(1);
