...
function renderForm(overrides: Partial<React.ComponentProps<typeof WordDetailsForm>> = {}) {
...
describe("WordDetailsForm", () => {
    ...
    describe("new word mode (word is null)", () => {
        ...
        it("calls onSave without wordId when Save is clicked with valid fields", async () => {
            ...
            await user.type(screen.getByPlaceholderText("The word you are learning"), "bonjour");
            await user.type(screen.getByPlaceholderText("What the word means"), "hello");
            await user.click(screen.getByRole("button", { name: "Save" }));
            expect(onSave).toHaveBeenCalledTimes(1);
        ...
        it("does not call onSave when Text is empty and user submits", async () => {
            const onSave = vi.fn();
            renderForm({ word: null, onSave });
            await user.type(screen.getByPlaceholderText("What the word means"), "meaning");
            await user.click(screen.getByRole("button", { name: "Save" }));
