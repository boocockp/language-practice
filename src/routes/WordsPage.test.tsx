...
describe("WordsPage", () => {
    beforeEach(() => {
        vi.mocked(useQuery).mockImplementation(((_query: unknown, args: unknown) => {
            ...
            if (args && typeof args === "object" && "wordId" in args && (args as { wordId?: Id<"words"> }).wordId) {
                return mockWord({
                    _id: (args as { wordId: Id<"words"> }).wordId,
                });
            }
            return [mockWord(), mockWord({ _id: "w2" as Id<"words">, text: "manger" })];
        }) as typeof useQuery);
    ...
    it("at /words/_new shows details form with New Word title and create mutation on Save", async () => {
        ...
        expect(screen.getByRole("heading", { name: "New Word" })).toBeInTheDocument();
        await user.type(screen.getByPlaceholderText("The word you are learning"), "test");
