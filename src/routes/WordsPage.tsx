...
import { WordDetailsForm, type ConfirmLeaveFn, type WordUpdatePayload } from "../features/words/WordDetailsForm";
    ...
    return (
        <section className="flex flex-col min-h-0 flex-1">
            <div className="flex items-center justify-between gap-2">
                ...
                <Button type="button" variant="primary" aria-label="Add word" onClick={handleAddClick}>
            ...
            {words === undefined ? (
                ...
                <div className={cn("mt-4 flex flex-1 min-h-0 gap-4", "flex-col md:flex-row")}>
                    {showTable && words && words.length > 0 && (
                        <div
                            className={cn("min-w-0 flex-1 md:max-w-md flex flex-col", showDetails && "hidden md:flex")}
                        >
                            <WordsTable words={words} selectedWordId={selectedWordId} onRowClick={handleRowClick} />
                        </div>
                    )}
                    {showDetails && (
                        <div className="min-w-0 flex-1 md:min-w-[320px] flex flex-col border border-slate-200 rounded-lg p-4 bg-white">
                            {isNewWord ? (
                                ...
                                <div className="space-y-2">
                                    <p className="text-slate-600">Word not found or you don’t have access to it.</p>
                                    <button type="button" onClick={goToWords} className="text-blue-600 hover:underline">
                                        Back to words
                                    </button>
                                </div>
                            ) : (
