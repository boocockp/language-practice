...
export function QuestionTypesPage() {
    ...
    useEffect(() => {
        if (!selectedQuestionTypeId || selectedQuestionTypeId === "_new" || !tableContainerRef.current) return;
        const row = tableContainerRef.current.querySelector(`[data-question-type-id="${selectedQuestionTypeId}"]`);
    ...
    return (
        <section className="flex flex-col min-h-0 flex-1">
            <div className="flex items-center justify-between gap-2">
                ...
                <Button type="button" variant="primary" aria-label="Add question type" onClick={handleAddClick}>
            ...
            {questionTypes === undefined ? (
                ...
                <Empty
                    className="mt-4"
                    title={user ? "No question types yet" : "Log in to manage your question types"}
                    description={
                        user
